...
sf::Vector2f RoundedRectangleShape::getPoint(std::size_t index) const
{
    ...
    // to make inverted rounded shape
    sf::Vector2f factor{ 1, -1 };
    switch (centerIndex) {
    case 0: center.x = mySize.x - myRadius; center.y = myRadius; factor = { -1, 1 }; break;
    case 1: center.x = myRadius; center.y = myRadius;  break;
    case 2: center.x = myRadius; center.y = mySize.y - myRadius; factor = { -1, 1 };  break;
    case 3: center.x = mySize.x - myRadius; center.y = mySize.y - myRadius; break;
    }


    return sf::Vector2f(factor.x * myRadius * sin(deltaAngle * (index - centerIndex) * pi / 180) + center.x,
                        factor.y * myRadius * cos(deltaAngle * (index - centerIndex) * pi / 180) + center.y);
